import { h as head, f as ensure_array_like, g as attr, k as store_get, t as escape_html, u as unsubscribe_stores, a as pop, l as bind_props, i as spread_attributes, n as spread_props, w as stringify, j as rest_props, v as value_or_fallback, x as mutate_store, p as push, m as sanitize_props, y as once } from './index2-DBlbVEXJ.js';
...
async function updateProcesses() {
  ...
  for await (const s of servers.data) {
    console.log(s.ip);
    s.host ? "/api/processes" : s.ip + "/api/processes";
...
function HostTag($$payload, $$props) {
  push();
  let { host } = $$props;
  $$payload.out += `<button${attr("class", ` flex flex-row gap-2 items-center curstor-pointer w-[150px] rounded-full m-auto border-2 p-1 ${stringify(host.visibile ? "border-success" : "border-neutral")}`, false)}><div${attr("class", `w-3 h-3 ${stringify(!host.pinged ? "bg-success animated-pulse" : "bg-neutral")}`, false)}></div> <div>${escape_html(host.ip)}</div></button>`;
  pop();
}
function _page($$payload, $$props) {
  ...
  $$payload.out += `<!--[-->`;
  AppHeader($$payload);
  $$payload.out += `<!--]--> <div class="flex flex-row items-center gap-2"><!--[-->`;
  if (servers.data) {
    const each_array = ensure_array_like(servers.data ?? []);
    $$payload.out += `<!--[-->`;
    for (let $$index = 0; $$index < each_array.length; $$index++) {
      const host = each_array[$$index];
      $$payload.out += "<!--[-->";
      $$payload.out += `<!--[-->`;
      HostTag($$payload, { host });
      $$payload.out += `<!--]-->`;
      $$payload.out += "<!--]-->";
    }
    $$payload.out += "<!--]-->";
  ...
  $$payload.out += ` <button class="btn">open modal</button></div> <dialog id="host_modal" class="modal"><!--[-->`;
  if (selectedHost) {
    $$payload.out += `<div class="modal-box"><h3 class="text-lg font-bold">Add Host</h3> <label class="form-control w-full max-w-xs"><div class="label"><span class="label-text">Host name</span></div> <input type="text"${attr("value", selectedHost.name, false)} placeholder="Type here" class="input input-bordered w-full max-w-xs"></label> <label class="form-control w-full max-w-xs"><div class="label"><span class="label-text">Host address</span></div> <input type="text"${attr("value", selectedHost.ip, false)} placeholder="Type here" class="input input-bordered w-full max-w-xs"></label> <div class="flex flex-row items-center justify-end gap-2"><button class="btn">Cancel</button> <button class="btn btn-success">Save</button></div></div>`;
    $$payload.out += "<!--]-->";
  } else {
    $$payload.out += "<!--]!-->";
  }
  ...
  if (processUI && processUI.length > 0) {
    ...
    const each_array_2 = ensure_array_like(processUI);
    $$payload.out += `<div class="flex flex-wrap md:hidden justify-center gap-5 mt-5"><!--[-->`;
    for (let $$index_1 = 0; $$index_1 < each_array_1.length; $$index_1++) {
      const process = each_array_1[$$index_1];
      $$payload.out += "<!--[-->";
      $$payload.out += `<!--[-->`;
    ...
    for (let $$index_3 = 0; $$index_3 < each_array_2.length; $$index_3++) {
      const process = each_array_2[$$index_3];
      $$payload.out += "<!--[-->";
      ...
      if (store_get($$store_subs ??= {}, "$settingsStore", settingsStore).showMetrics) {
        ...
        if (process.monitor) {
          const each_array_3 = ensure_array_like(Object.entries(process?.monitor).filter(([key, value]) => !ToExludeMonitorKeys.includes(key) && value.value !== void 0).map(([key, value]) => {
            return { key, value: value.value };
          }));
          $$payload.out += `<!--[-->`;
          for (let $$index_2 = 0; $$index_2 < each_array_3.length; $$index_2++) {
            const metric = each_array_3[$$index_2];
            $$payload.out += "<!--[-->";
            $$payload.out += `<div class="text-xs italic text-left">${escape_html(metric.key)}:</div> <div class="text-left">${escape_html(metric.value)}</div>`;
            $$payload.out += "<!--]-->";
          }
...
//# sourceMappingURL=_page.svelte-Blf2-Hfx.js.map
